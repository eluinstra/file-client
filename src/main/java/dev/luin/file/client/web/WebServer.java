...
public class WebServer implements Config, SystemInterface
{
	...
	private ServerConnector createHttpsConnector(CommandLine cmd, Server server, SslContextFactory.Server factory)
	{
		...
		connector.setPort(Integer.parseInt(cmd.getOptionValue(Option.PORT.name) == null ? DefaultValue.PORT.value : cmd.getOptionValue(Option.PORT.name)));
